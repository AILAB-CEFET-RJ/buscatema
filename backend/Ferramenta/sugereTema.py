...
def create_embedding(file, indice, corpus, num, model, data_type):
    ...
    nameEmbedding = createFileName(file.split('/')[-1].split('.')[0])
    if(data_type == 'tema'):
...
def main():
    ...
    appeal_file = '/uploads/recurso1.csv'
    themes_file = '/uploads/lista_temas.csv'
    
    # Processa texto do recurso
    corpus_recurso = process_corpus(appeal_file, 'recurso')
    
    # Processa lista de temas 
    corpus_tema = process_corpus(themes_file, 'tema')
    ...
    arquivo_embedding_recurso = create_embedding(appeal_file, corpus_recurso['indice'], conteudo_recurso, corpus_recurso['num_tema_cadastrado'], model, 'recurso')
    
    # Cria embedding temas
    conteudo_tema = corpus_tema['tema'].tolist()
    arquivo_embedding_tema = create_embedding(themes_file, corpus_tema['indice'], conteudo_tema, corpus_tema['num_tema_cadastrado'], model, 'tema')
